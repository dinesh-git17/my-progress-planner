      ...
      fetch(`/api/admin/summaries?timestamp=${timestamp}`)
        ...
        .then((data) => {
          ...
          const users = Array.from(
            new Set(
              summariesData.map((s: DailySummary) => s.name || 'Unknown'),
            ),
          ) as string[];
          const dates = Array.from(
            new Set(summariesData.map((s: DailySummary) => s.date)),
          ) as string[];
                    ...
                    {(['breakfast', 'lunch', 'dinner'] as const).map(
                      (mealType) => {
                        const summaryText = summary[
                          `${mealType}_summary` as keyof DailySummary
                        ] as string;
